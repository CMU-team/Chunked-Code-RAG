...
# ANLP Project - added deduplication here      
def get_top_docs(results: dict, corpus: dict, task_id: str, topk: int = 10) -> list[str]:
    ...
    # Initialize a set to keep track of unique code_ids
    unique_code_ids = set()
    # Initialize the resulting list
    unique_doc_scores_sorted = []
    
    for code_id, score in doc_scores_sorted:
        if code_id not in unique_code_ids:
            unique_code_ids.add(code_id)
            unique_doc_scores_sorted.append((code_id, score))
        if len(unique_doc_scores_sorted) == topk:
            break  # Stop once we have the top-k unique code_ids
    
    # Extract corresponding code snippets
    doc_code_snippets = [corpus[code_id] for code_id, score in unique_doc_scores_sorted]
